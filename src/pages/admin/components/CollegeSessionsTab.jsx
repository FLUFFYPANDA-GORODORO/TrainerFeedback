...
import { format, startOfDay, endOfDay, subDays, subMonths, startOfMonth, endOfMonth, isWithinInterval, parse } from 'date-fns';
...
const CollegeSessionsTab = () => {
  ...
  const getDateRange = (range) => {
    ...
    switch (range) {
      ...
      case 'thisMonth':
        // Modified to include end of month for future scheduled sessions
    ...
    return sessions.filter(session => {
      ...
      if (filters.dateRange !== 'all') {
        ...
        if (range && session.sessionDate) {
          ...
          const sessionDate = parse(session.sessionDate, 'yyyy-MM-dd', new Date());
          matchDate = isWithinInterval(sessionDate, range);
        }
      }

      return matchSearch && matchCourse && matchDept && matchYear && matchBatch && matchTrainer && matchDate;
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <Button 
              ...
              size="sm" 
              onClick={() => {
                setFilters({ course: 'all', department: 'all', year: 'all', batch: 'all', trainer: 'all', dateRange: 'all' });
                setSearchQuery('');
              }} 
              className="gap-2"
            >
                ...
                <TableRow key={session.id} className="hover:bg-muted/50 transition-colors">
                  ...
                  <TableCell>
                    ...
                    <div className="text-xs text-muted-foreground">
                      Year {session.year} • Batch {session.batch} • {session.academicYear}
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">{session.assignedTrainer?.name || 'Unassigned'}</span>
                    </div>
                  </TableCell>
                  <TableCell>
